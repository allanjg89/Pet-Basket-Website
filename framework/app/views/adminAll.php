<div class="container">
    <div class="row">
        <div class = "col-md-2"><h4>All Pets</h4></div>
        <div class="col-md-3">
            <h4>
                <em>
                    <?php
                    $totalAll = count($passedToView['allPets']);
                    echo $totalAll . " results";
                    ?>
                </em>
            </h4>
        </div>
    ...
    <?php
    $user = USER;
    if (isset($passedToView['allPets']) && is_array($passedToView['allPets'])) {
        $page = 1;
        foreach ($passedToView['allPets'] as $pet) {
            if (isset($pet->images) && $pet->images !== null) {
                foreach ($pet->images as $image) {
                    ...
                    $msgData = '';
                    if (isset($_SESSION['user']) && isset($browser)) {
                        $browserId = $browser->getId();
                        $msgData = "$browserId#$petId";
                    }

                    echo <<< img
                                        
    <div id="$petId" class="panel panel-default petPanel">
	<div id="page_A_$page" class="panel-heading">
	    <div class="row">
		<div class="col-md-6">
		    <div class="petName">
			$petName
		...
		<div class="col-md-2">
		    ...
		    <button name="adminRemove" id="adminRemove" type="submit" class="btn btn-lrg btn-primary fr adminRemove" data-user="0#$petId" data-toggle="modal" data-target="#contactPosterModal">Remove</button>    
		</div>
	    </div>
	</div>
